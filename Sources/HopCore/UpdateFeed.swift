...
public enum UpdateFeed {
    ...
    /// Whether the release being offered is later than the one running.
    /// Compared as numbers per component, never as text: 1.10.0 stands above
    /// 1.9.1 while the strings sort the other way round, and every Mac's update
    /// to 1.10.0 rides on that. Anything that is not a version answers false —
    /// a build is not replaced on a reading nobody can make sense of.
