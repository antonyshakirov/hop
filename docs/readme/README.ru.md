...
Строка модуля открывает окно, и бросать нужно именно в него. Бросьте архив —
он распакуется там же, где лежит; бросьте файлы или папки — рядом появится
один архив. zip, tar, tar.gz, tar.bz2, tar.xz и gz работают сразу; для rar и
7z при первой встрече докачивается небольшой помощник (~6 МБ) с проверкой
подписи. rar Hop распаковывает, но никогда не создаёт — формат проприетарный.
Включите «открывать архивы через Hop» — и двойной клик по архиву распакует его
через Hop, видно модуль в панели или нет.
...
открыв панель, или удержав esc три секунды. Короткое нажатие кнопки питания
