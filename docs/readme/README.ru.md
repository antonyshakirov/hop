...
Бросьте архив на панель — он распакуется там же, где лежит; бросьте файлы или
папки — рядом появится один архив. zip, tar, tar.gz, tar.bz2, tar.xz и gz
работают сразу; для rar и 7z при первой встрече докачивается небольшой
помощник (~6 МБ) с проверкой подписи. rar Hop распаковывает, но никогда не
создаёт — формат проприетарный.
...
Два включаемых модуля для дизайнеров и разработчиков. Пипетка кладёт любой
цвет с экрана в историю буфера как hex, rgb или hsl — разрешение на запись
экрана ей не нужно. «Текст с экрана» выделяет область и отправляет текст или
qr-код из неё в ту же историю, уже скопированным и доступным поиску.

### Блокировка клавиатуры

Одна кнопка — и клавиатура перестаёт отвечать, чтобы её можно было протереть,
не выключая Мак и не закрывая крышку. Выход — мышью или по таймеру на
30 секунд, 1 минуту или 5 минут.
...
- **запись экрана** — только модуль «текст с экрана»; пипетке она не нужна
