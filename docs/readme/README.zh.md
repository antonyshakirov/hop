...
模块那一行会打开一个窗口，拖放就在那个窗口里进行。拖入压缩包，它就在原处解压；拖入文件或文件夹，
旁边就生成一个压缩包。zip、tar、tar.gz、tar.bz2、tar.xz 和 gz 开箱即用；遇到 rar 和 7z 时会在第一次
下载一个约 6 MB 的小助手，并校验签名。Hop 能解 rar，但从不创建它 —— 这个格式是专有的。打开「用 Hop
打开压缩包」后，双击压缩包就由 Hop 解压，无论模块在面板里是否可见。
...
长按 esc 三秒。电源键的短按同样被吞掉；长按仍会强制关机，因为那是硬件负责的。
