...
网页同样可以转换：粘贴一个网址或拖入一份保存的网页，输出为 PDF、docx、
Markdown、RTF 或纯文本。
...
框选一块区域、拍下指针所在的窗口或整块屏幕，编辑器会带着画面直接打开。用铅笔、荧光笔、箭头、图形、编号步骤和文字标注，把不该看到的内容藏进模糊或马赛克里，也可以把除了你所指之处以外的一切都模糊掉。然后保存文件或复制到剪贴板。
...
顶部的 `downloads` 徽章把每个发行版每个文件的下载次数加在一起——Apple Silicon 和
Intel 的磁盘映像，连同 zip 构建。每一次下载都算数，所以手动下载了五个版本的
Mac 就记五次：应用在那台机器上出现了五回。自动更新不在这个数里，更新程序从 hop.tools 取，
从不碰发行版的文件；随 zip 一起下载的签名文件也不算。计数由 GitHub 自己按文件保存，
因此得出这个数字不会测量你的任何信息：应用不发送任何标识，本站的日志也不会进入这个数字。
