...
[Bahasa Indonesia](README.id.md) · [Deutsch](README.de.md) · [English](../../README.md) · [Español](README.es.md) · [Français](README.fr.md) · [Italiano](README.it.md) · [Nederlands](README.nl.md) · [Polski](README.pl.md) · [Português](README.pt.md) · [Türkçe](README.tr.md) · [Tiếng Việt](README.vi.md) · [Русский](README.ru.md) · [हिन्दी](README.hi.md) · [ไทย](README.th.md) · **한국어** · [中文](README.zh.md) · [日本語](README.ja.md)
...
여러 작업이 동시에 돌아갈 수 있고, 메뉴 막대에는 마지막에 시작한 시계가 보인다.
...
웹 페이지도 변환된다: 주소를 붙여 넣거나 저장한 페이지를 떨어뜨리면 PDF, docx,
Markdown, RTF 또는 일반 텍스트로 나온다.
...
통화, 녹화, 리뷰 중에 화면 위에 바로 그리세요: 연필, 아래 글자가 읽히는 넓은 형광펜, 화살표, 도형, 번호 매긴 단계. 사라지는 잉크는 포인터를 뗀 뒤 몇 초 만에 스스로 사라집니다. 돋보기와 흐리기는 움직이는 화면 위에서도 동작합니다. 세부를 확대해 통화 참가자 모두에게 보여 주거나, 있어서는 안 될 이름을 가릴 수 있습니다.

스위치 하나로 아래 앱에 클릭을 돌려주면서 표시는 화면에 남길 수 있습니다. 「저장」과 「복사」는 화면과 그림을 함께 담습니다. 화면 전체를 공유할 때 상대에게 표시가 보입니다. 창 하나만 공유하면 macOS가 그 창만 합성합니다.
...
Bahasa Indonesia, Deutsch, English, Español, Français, Italiano, Nederlands, Polski, Português, Türkçe, Tiếng Việt, Русский, हिन्दी, ไทย, 한국어, 中文, 日本語 – 앱은 처음부터 시스템 언어를 그대로
따라갑니다.
...
1.10.0으로 업데이트하면 모든 권한이 한 번 지워지고 다시 요청된다. 권한은 코드 서명에
묶이는데, Apple이 서명하면서 Hop의 서명이 바뀌었다. 이전 서명에 준 권한은 목록에
남아 있어도 더는 작동하지 않았다. 1.10.0부터는 업데이트를 견딘다.
