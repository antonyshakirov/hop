...
시스템 모니터, 클립보드 히스토리, 파일 변환기, 창 관리자, 가벼운 토렌트
클라이언트, 화면 캡처, 화면에 그리기, 텍스트 인식 등 더 많은 기능. 필요한 것만 켜서 아이콘의 최대 네 개 탭에 나눠 담습니다.
...
⭐ Hop이 Homebrew 공식 카탈로그에 들어가려면 별 225개가 필요하고, 별 하나하나가 그 목표에 더 가까워지게 합니다.
...
모듈 옆의 전원 버튼은 모듈을 끕니다. 자리는 그대로 두고 동작만 멈춥니다 – 단축키도, 표시도, 백그라운드 수집도 없습니다.
...
여러 작업이 동시에 돌아갈 수 있고, 메뉴 막대에는 마지막에 시작한 시계가 보입니다.
...
실행 중에도 그 변경을 읽어들입니다. 그래서 에이전트든, 스크립트든, 텍스트 편집기를 연
당신이든 작업을 덧붙이고 그것이 나타나는 모습을 볼 수 있습니다. 명령 파일 실행과 `hop://` 링크도 지원하므로 같은
...
웹 페이지도 변환됩니다: 주소를 붙여 넣거나 저장한 페이지를 떨어뜨리면 PDF, docx,
...
표나 코드 조각도 읽을 수 있습니다. 인식은 Apple의 Vision이며 전부 이 Mac 안에서 이뤄집니다.

인식 결과에 웹 주소가 있으면 「링크 열기」 버튼이 나타납니다. 청구서 QR 코드의 링크가 휴대폰
없이 브라우저에서 바로 열립니다. 웹 주소만 해당합니다. 스캔한 코드는 외부에서 온 입력이므로
...
영역을 잡거나, 포인터 아래의 창 또는 화면 전체를 찍으면 편집기가 이미지를 담은 채 열립니다. 연필, 형광펜, 화살표, 도형, 번호 매긴 단계, 텍스트로 표시하고, 보이면 안 되는 것은 흐리게나 모자이크로 가리세요. 가리키는 곳만 남기고 나머지를 흐리게 할 수도 있습니다. 그런 다음 파일로 저장하거나 클립보드에 복사합니다.
...
통화, 녹화, 리뷰 중에 화면 위에 바로 그리세요: 연필, 아래 글자가 읽히는 넓은 형광펜, 화살표, 도형, 번호 매긴 단계. 사라지는 잉크는 포인터를 뗀 뒤 몇 초 만에 스스로 사라지므로, 말하다 말고 지울 필요가 없습니다. 돋보기와 흐리기는 움직이는 화면 위에서도 동작합니다. 세부를 확대해 통화 참가자 모두에게 보여 주거나, 있어서는 안 될 이름을 가릴 수 있습니다.
...
아무것도 열지 않고 연결하고 끊습니다. 터널이 서 있는 동안에는 메뉴 막대 아이콘 모서리에 작은 점이 다른 표시들과 나란히 켜지므로 패널을 닫아 둔 채로도 보입니다. 무언가 지나가는 동안에는 초록, 터널은 켜져 있는데 아무것도 돌아오지 않으면 주황이 되어 조용히 죽은 연결이 멀쩡해 보이지 않습니다. 어느 줄인지는 패널이 표시합니다. 이름을 누르면 국가를 고르거나 설정을 바꿔야 할 때를 위해 그 VPN의 창이 열리고, 창을
닫으면 Hop이 앱을 종료합니다. 일주일에 두 번 만지는 스위치 때문에 앱이 Dock과 메뉴 막대에 계속 머물 일은 없습니다. 연결은 유지됩니다 – 터널을 붙잡는 것은 앱이 아니라
시스템이니까요.

행에는 클라이언트가 스스로 알린 것만 나옵니다: 이름과, 괄호 안에 구성이 덧붙이는 것,
보통은 국가입니다. 서버 주소로 국가를 추측하지는 않습니다. 주소 등록부가 말해주는 것은
대역이 어디에 등록됐는지이지 장비가 어디 있는지가 아니며, 틀린 국가를 자신 있게 보여 주는
것은 아무것도 보여 주지 않는 것보다 나쁩니다.
...
Bahasa Indonesia, Deutsch, English, Español, Français, Italiano, Nederlands, Polski, Português, Türkçe, Tiếng Việt, Русский, Српски, עברית, العربية, فارسی, اردو, हिन्दी, ไทย, 한국어, 中文, 日本語 – 앱은 처음부터 시스템 언어를 그대로
따라갑니다.
...
위의 `downloads` 배지는 모든 릴리스 파일의 내려받기를 전부 더한 수입니다. Apple Silicon과
Intel 디스크 이미지, 그리고 zip 빌드를 함께 셉니다. 내려받기는 한 번씩
모두 세므로, 다섯 버전을 손수 받은 Mac은 다섯이 됩니다. 그 컴퓨터에 앱이
다섯 번 도착했기 때문입니다. 자동 업데이트는 이 수에 들어가지 않습니다. 업데이터는
hop.tools에서 받아 오고 릴리스 파일은 건드리지 않기 때문입니다. zip에
딸려 오는 서명 파일도 세지 않습니다. 집계는 GitHub이 파일별로 직접 하므로, 이
숫자를 만들기 위해 당신에 대해 측정하는 것은 없습니다. 앱은 어떤 식별자도 보내지
않고, 이 사이트의 기록도 숫자에 들어가지 않습니다.

## 권한

Hop은 그 권한이 필요한 기능을 실제로 쓸 때만 권한을 요청하며, 앱의 설정 창에
모든 권한과 현재 상태가 정리되어 있습니다:

- **네트워크 – hop.tools** – 업데이트 확인과 내려받기, 그리고 선택 도구
...
- **화면 기록** – 영역을 잡을 때의 텍스트 인식, 스크린샷, 그리기 레이어의 돋보기와 흐리기에
  필요합니다. 색상 스포이트에는 필요 없습니다
...
분석도, 텔레메트리도, 계정도, 크래시 리포트도 없습니다. hop.tools에는 새 버전이 있는지
...
보세요. 앱의 설정 창에는 «앱 권한» 페이지가 있어 같은 목록과 각 권한의 현재 상태를 보여 줍니다.

1.10.0으로 업데이트하면 모든 권한이 한 번 지워지고 다시 요청됩니다. 권한은 코드 서명에
묶이는데, Apple이 서명하면서 Hop의 서명이 바뀌었습니다. 이전 서명에 준 권한은 목록에
남아 있어도 더는 작동하지 않았습니다. 1.10.0부터는 권한이 업데이트 후에도 유지됩니다.
