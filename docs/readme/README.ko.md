...
모듈의 줄이 창을 열고, 파일은 그 창에 놓습니다. 압축 파일을 놓으면 있던 자리에서 그대로 풀리고,
파일이나 폴더를 놓으면 그 옆에 압축 파일 하나가 생깁니다. zip, tar, tar.gz, tar.bz2, tar.xz, gz는
바로 됩니다. rar과 7z는 처음 만났을 때 약 6 MB짜리 작은 도우미를 서명 확인 후 내려받습니다. Hop은
rar을 풀지만 만들지는 않습니다. 독점 형식이기 때문입니다. «Hop으로 압축 파일 열기»를 켜면 더블
클릭한 압축 파일이 Hop으로 풀립니다. 모듈이 패널에 보이든 아니든 마찬가지입니다.
...
방법은 넷입니다: 덮개의 버튼, 패널의 버튼, 패널 열기, esc 3초 길게 누르기. 전원 키의 짧은 누름도
