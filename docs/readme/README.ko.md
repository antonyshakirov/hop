...
압축 파일을 패널에 놓으면 그 자리에서 풀리고, 파일이나 폴더를 놓으면 그 옆에
압축 파일 하나가 생깁니다. zip, tar, tar.gz, tar.bz2, tar.xz, gz는 바로
동작하고, rar과 7z은 처음 만났을 때 서명을 확인한 작은 도우미(약 6 MB)를 한 번
받아 옵니다. Hop은 rar을 풀지만 만들지는 않습니다 — 독점 형식이니까요.
...
디자이너와 개발자를 위한, 직접 켜는 두 모듈입니다. 스포이트는 화면의 어떤 색이든
hex, rgb, hsl로 클립보드 기록에 넣습니다 — 화면 기록 권한은 필요 없습니다. 화면
텍스트는 영역을 잡아 그 안의 텍스트나 QR 코드를 같은 기록에, 이미 복사된 채로
남깁니다.

### 키보드 잠금

버튼 하나로 키보드 전체가 반응을 멈춰, Mac을 끄거나 덮개를 닫지 않고도 닦을 수
있습니다. 빠져나오는 방법은 마우스, 또는 30초·1분·5분 타이머입니다.
...
- **화면 기록** — 화면 텍스트 모듈에만 해당. 스포이트에는 필요 없습니다
