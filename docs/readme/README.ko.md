...
맨 위의 `downloads` 배지는 사람이 아니라 클릭을 셉니다. 다운로드 버튼과 Homebrew
포뮬러는 hop.tools를 거치는데, hop.tools는 여느 사이트가 페이지 조회를 기록하듯
요청을 웹 서버 로그에 적고 GitHub 릴리스의 파일로 넘겨 줍니다. 로그에 남는 것은
주소의 솔트 해시뿐이라, 한 기기에서 두 번 눌러도 한 번으로 세고 주소 자체는 저장되지
않습니다. 이 모든 것은 앱과 무관합니다. Hop이 설치되기 전에 집계되며, 업데이트를
한 번도 확인하지 않는 사본도 똑같이 셉니다.
