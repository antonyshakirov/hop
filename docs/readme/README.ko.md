...
**macOS 메뉴 막대에 사는 작은 동반자: 타이머, 잠자기 방지,
시스템 모니터, 클립보드 히스토리, 파일 변환기, 창 관리자,
그리고 가벼운 토렌트 클라이언트. 클릭 한 번이면 필요한 모든 것이
바로 그 자리에.**
...
Hop은 Mac의 메뉴 막대에 자리 잡고 자잘한 유틸리티 대여섯 개를 대신합니다.
뽀모도로식 타이머, caffeinate 스타일 잠자기 차단, 시스템 모니터,
클립보드 관리자, 드래그 앤 드롭 파일 변환기, 창 스냅 기능,
그리고 가벼운 토렌트 클라이언트까지 — 일곱 개의 앱 대신 가벼운
네이티브 앱 하나면 충분합니다.
...
첫 실행 시: `Hop.app`을 오른쪽 클릭 → **열기** → 확인
(앱은 아직 공증되지 않았습니다). macOS 14 이상이 필요합니다.
...
그리고 재생 중인 미디어까지 멈춰 줄 수 있는 종료 알림.
...
최근에 복사한 100개(최대 300개)의 항목(텍스트와 이미지)을 클릭 한 번으로 다시 복사하거나
이전 앱에 바로 붙여넣기. 암호를 비롯한 가려진 입력은 절대 저장되지
않습니다.
...
테마, 전역 단축키, 로그인 시 실행, 그리고 크래시 루프에서 앱을 복구하는
안전 모드.
...
토렌트 트래픽 자체를 주고받을 때뿐입니다. 업데이트와 토렌트 엔진은
서명된 아카이브로 배포되며 설치 전에 Ed25519 서명으로 검증됩니다.

웹사이트: [antonshakirov.com/products/hop](https://www.antonshakirov.com/products/hop)

## 무료인 이유

Hop은 완전히 무료입니다. 체험판도, 프로 버전도, 인앱 구매도 없습니다. 광고도, 데이터 수집도, 계정도 없어 수익화할 것도, 팔 것도 없습니다. 개인 프로젝트입니다. 제가 쓰려고 Hop을 만들어 매일 사용하고 있고, 그저 공유할 뿐입니다. 쓸모가 있다면 다른 사람에게도 알려 주세요.
