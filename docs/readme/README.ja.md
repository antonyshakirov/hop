...
モジュールの行がウインドウを開き、ドロップはそのウインドウで行います。アーカイブを落とせば置かれた
場所でそのまま展開し、ファイルやフォルダを落とせばその隣に 1 つのアーカイブができます。zip・tar・
tar.gz・tar.bz2・tar.xz・gz はそのまま動作。rar と 7z は初回に約 6 MB の小さなヘルパーを署名検証
つきでダウンロードします。Hop は rar を展開しますが作成はしません（プロプライエタリな形式のため）。
「アーカイブを Hop で開く」を有効にすると、ダブルクリックした書庫は Hop で展開されます。モジュールが
パネルに出ていなくても同じです。
...
— カバーのボタン、パネルのボタン、パネルを開くこと、esc の 3 秒長押し。電源キーの短押しも飲み込まれ
