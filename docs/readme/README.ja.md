...
パネルにアーカイブを落とすと、置かれている場所でそのまま展開されます。ファイルや
フォルダを落とせば、その隣にアーカイブができます。zip、tar、tar.gz、tar.bz2、
tar.xz、gz はそのまま扱えます。rar と 7z は初回だけ、署名を検証した小さなヘルパー
（約 6 MB）を取得します。Hop は rar を展開しますが、作成はしません — 専有形式
だからです。
...
デザイナーと開発者向けの、自分で有効にする 2 つのモジュール。スポイトは画面上の
どんな色でも hex・rgb・hsl としてクリップボード履歴に入れます（画面収録の許可は
不要）。画面の文字は範囲を囲み、その中の文字や qr コードを同じ履歴に、コピー済みの
状態で残します。

### キーボードロック

ボタンひとつでキーボード全体が反応しなくなり、Mac を終了したり蓋を閉じたりせずに
拭けます。解除はマウス、または 30 秒・1 分・5 分のタイマーで。
