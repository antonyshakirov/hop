...
Hop は Mac のメニューバーに常駐し、こまごました 6 つのユーティリティを
まとめて置き換えます。ポモドーロ式タイマー、caffeinate 風のスリープ
ブロッカー、システムモニター、クリップボードマネージャー、ドラッグ＆
ドロップのファイル変換、ウィンドウスナップ——6 本のアプリの代わりに、
軽量なネイティブアプリが 1 本。
...
コピーした直近 100 件（最大 300 件）を保持。ワンクリックでコピーし
...
アカウントなし。アプリがネットワークに触れるのはアップデートの確認時と、
内蔵スピードテストを実行した時だけ。アップデートは署名付きアーカイブで
配信され、インストール前に Ed25519 署名で検証されます。
