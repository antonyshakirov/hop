...
上部の `downloads` バッジが数えるのはクリックで、人ではありません。ダウンロードボタンと
Homebrew のフォーミュラは hop.tools を経由します。hop.tools は、どのサイトもページの閲覧を
記録するのと同じようにリクエストをウェブサーバーのログに書き、GitHub リリースのファイルへ
リダイレクトします。ログに残るのは接続元アドレスにソルトを加えたハッシュなので、同じマシンから
の 2 回のクリックは 1 回と数えられ、アドレスそのものは保存されません。アプリはこれに一切
関わりません。計測は Hop をインストールする前に行われ、アップデートを一度も確認しないコピーも
同じように数えられます。
